...
public class RanSanMoi extends JFrame{
    ...
    public RanSanMoi(){
        super("RanSanMoi");   // tiêu đề cửa sổ 
...
//        setDefaultCloseOperation();
    ...
    private class Handler implements KeyListener{            // khai báo lớp nằm trong lớp có đặc điểm KeyListener
        ...
        public void keyPressed(KeyEvent e) {                  // sự kiện ấn 
            if(e.getKeyCode() == KeyEvent.VK_ENTER){       // lắng nghe sự kiện ấn nút space
            ...
            if(e.getKeyCode() == KeyEvent.VK_UP||e.getKeyCode()== KeyEvent.VK_E){     // ấn mũi tên lên để đi lên
                game.ran.setVector(Snake.GO_UP);
            }
            
            if(e.getKeyCode() == KeyEvent.VK_DOWN||e.getKeyCode()== KeyEvent.VK_X){    // ấn mũi tên xuống để đi xuống
                game.ran.setVector(Snake.GO_DOWN);
            }
            
            if(e.getKeyCode() == KeyEvent.VK_LEFT||e.getKeyCode()== KeyEvent.VK_S){    // ấn mũi tên sang trái để đi sang trár
                game.ran.setVector(Snake.GO_LEFT);
            }
            
            if(e.getKeyCode() == KeyEvent.VK_RIGHT||e.getKeyCode()== KeyEvent.VK_D){    // ấn mũi tên sang phải để đi sang phải
